...
module Yell # :nodoc:
  ...
  class Formatter
    ...
    LegacyTable = Table.keys.to_h { |k| [k, 'noop'] }.merge(
      'm' => 'message(msg)',
      'l' => 'level(event, 1)',
      'L' => 'level(event)',
      'd' => 'date(time)',
    ...
    class Modifier
      ...
      def call(message)
        mod = @repository[message.class] || @repository[message.class.to_s]
    ...
    def define_date_method!
      ...
      instance_eval <<-RUBY, __FILE__, __LINE__ + 1
        def date(t = Time.now)  # def date(t = Time.now)
          #{buf}                #   t.strftime(@date_pattern)
        end                     # end
      RUBY
    end
