...
describe Yell::Formatter do
  ...
  let(:pattern) { '%m' }
  let(:formatter) { described_class.new(pattern) }
  ...
  describe 'patterns' do
    ...
    context ':caller' do
      ...
      before do
        allow(event).to receive(:file).and_return('/path/to/file.rb')
        allow(event).to receive(:line).and_return('123')
        allow(event).to receive(:method).and_return('test_method')
      end
  ...
  describe 'presets' do
    ...
    context 'DefaultFormat' do
      ...
      it 'returns correctly' do
        expect(output).to eq("#{time.iso8601} [ INFO] #{$PROCESS_ID} : Hello World!\n")
      end
    end
    ...
    context 'ExtendedFormat' do
      ...
      it 'Returns correctly' do
        expect(output).to eq("#{time.iso8601} [ INFO] #{$PROCESS_ID} #{Socket.gethostname} : Hello World!\n")
      end
    end
  end
  ...
  describe 'custom message modifiers' do
    let(:formatter) do
      described_class.new(pattern) { |f| f.modify(String) { |m| "Modified! #{m}" } }
    end
